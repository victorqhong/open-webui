...
from open_webui.routers import openai, ollama, azure
...
async def get_all_base_models(request: Request):
    ...
    if request.app.state.config.ENABLE_AZURE_API:
        azure_models = await azure.get_all_models(request)
        azure_models = azure_models["data"]

    function_models = await get_function_models(request)
    models = function_models + openai_models + ollama_models + azure_models
